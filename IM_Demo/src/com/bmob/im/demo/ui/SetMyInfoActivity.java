...
public class SetMyInfoActivity extends ActivityBase implements OnClickListener {
	...
	private void initView() {
		...
		btn_chat = (Button) findViewById(R.id.btn_chat);// 发起会话
		btn_black = (Button) findViewById(R.id.btn_back);// 添加至黑名单
		btn_add_friend = (Button) findViewById(R.id.btn_add_friend);
		btn_add_friend.setEnabled(false);
		btn_chat.setEnabled(false);
		btn_black.setEnabled(false);
		if (from.equals("me")) {// 如果是用户查看自身的资料
			...
			initOtherData(username);//如果是查看好友或者陌生人资料才执行此句，查看自己的资料不会执行（在else语句内的）
		}
	}
	...
	/**
	 * 测试关联关系是否可用 @Title: addBlog @Description: TODO @param @return void @throws
	 */
	public void addBlog() {
